...
class FFTLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        pred_fft = torch.stack([pred_fft.real, pred_fft.imag], dim=-1) # 实部和虚部imaginary
...
@LOSS_REGISTRY.register()
class ReweightedL1(nn.Module):
    def __init__(self, alpha_sp=1, gamma_sp=1, alpha_lp=1, gamma_lp=1,
                 loss_weight=1.0,
                 angRes=5, scale_factor=4):
        super(ReweightedL1, self).__init__()
        self.alpha_sp = alpha_sp
        self.gamma_sp = gamma_sp
        self.alpha_lp = alpha_lp
        self.gamma_lp = gamma_lp
        self.loss_weight = loss_weight
        # self.criterion = torch.nn.L1Loss(reduction=reduction)
        self.angRes = angRes
        self.scale_factor = scale_factor

    def compute_weights(self, z, alpha, gamma):
        # Harmonize z
        g_z = (z - z.min()) / (z.max() - z.min())
        # Compute weights
        W_z = alpha * torch.exp(gamma * g_z)
        return W_z

    def compute_y_sp(self, x_bi, y_hr):
        return torch.abs(input=(x_bi - y_hr))

    def compute_y_lp(self, pred, target):
        return torch.abs(input=(pred - target))

    def forward(self, x, pred, target):
        x_bi = interpolate(x, angRes=self.angRes, scale_factor=self.scale_factor, mode='bicubic')
        y_sp = self.compute_y_sp(x_bi, target)
        y_lp = self.compute_y_lp(pred, target)
        w_sp = self.compute_weights(y_sp, self.alpha_sp, self.gamma_sp)
        w_lp = self.compute_weights(y_lp, self.alpha_lp, self.gamma_lp)
        # print(self.criterion(pred, target)) tensor(0.0675, device='cuda:0', grad_fn=<MeanBackward0>)
        # loss = w_sp * w_lp * self.criterion(pred, target)
        loss = (w_sp * w_lp * torch.abs(pred - target)).mean()
